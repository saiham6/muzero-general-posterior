...
class MCTS:
    ...
    """
    #########################################################
    Gaussian Thompson Sampling implementation Start from Here 
    Saiham Rahman. ID: 210769593
    #########################################################
    """
    def select_child(self, node, min_max_stats):
        ...
        """
        actions = [self.thompson(node, child, min_max_stats) for action, child in node.children.items()]
        actions_arms = [action for action, child in node.children.items()]
        action = actions_arms[numpy.argmax(actions)]

        return action, node.children[action]

    @staticmethod
    def t_normal(mean, precision):
        return numpy.random.normal(mean, numpy.sqrt(1 / precision))

    def thompson(self, parent, child, min_max_stats):

        prior_score = self.t_normal(child._mean, child._precsion)
    ...
    def backpropagate(self, search_path, value, to_play, min_max_stats):
        ...
        if len(self.config.players) == 1:
            for node in reversed(search_path):
                ...
                fPrec = 1/(node._Fstd**2)

                node._mean = ((fPrec*value) + (node._precsion*node._mean) ) / (fPrec + node._precsion)
                node._precsion = node._precsion + fPrec
...
class Node:
    def __init__(self, prior, config):
        ...
        self.reward = 0
        self._mean = 0
        self._Fstd = self.config.Fstd
        self._gauss_std = self.config.g_std
        self._precsion = 1.0 / self._gauss_std**2

    """
    #########################################################
    Gaussian Thompson Sampling implementation End Here 
    Saiham Rahman. ID: 210769593
    #########################################################
    """
    ...
    def expand(self, actions, to_play, reward, policy_logits, hidden_state):
        ...
        for action, p in policy.items():
            self.children[action] = Node(p,self.config)
