...
class MuZero:
    ...
    def save_to_file(self, arr):
        numpy.save(self.config.results_path / "reward_history.npy", arr, allow_pickle=True)
    ...
    def logging_loop(self, num_gpus):
        ...
        try:
            arr = numpy.array([0])
            while info["num_played_games"] < self.config.training_steps:
                info = ray.get(self.shared_storage_worker.get_info.remote(keys))
                writer.add_scalar(
                ...
                print(
                    f'Last test reward: {info["total_reward"]:.2f}. Counter step: {counter}/{self.config.training_steps}. Training step: {info["training_step"]}/{self.config.training_steps}. Played games: {info["num_played_games"]}. Loss: {info["total_loss"]:.2f}',
                    end="\r",
                )
                arr = info["rewards_backup"]
                if counter % 5 == 0:
                    self.save_to_file(arr)
                counter += 1
                time.sleep(0.5)
            load_npy = numpy.load(self.config.results_path / "reward_history.npy", allow_pickle=True)
            print("\n", load_npy.shape[0])
            load_npy = load_npy[-1000:]
            first = numpy.sum(numpy.cumsum(load_npy))
            last = numpy.sum(numpy.cumsum(numpy.full(load_npy.shape[0],numpy.max(load_npy))[-1000:]))
            t_regret = last - first
            print(
                f'\nlast 1000 games regret: {t_regret}',
                end="\n", flush = True
            )
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print("\nWelcome to MuZero! Choose the Algorithm: ")
        options = [
            "pUCT",
        ...
        choice = input("Enter a number to choose an algorithm: ")
        valid_inputs = [str(i) for i in range(len(options))]
        while choice not in valid_inputs:
            choice = input("Invalid input, enter a number listed above: ")
        choice = int(choice)
        if choice == 0:
            import self_play_OG as self_play
        elif choice == 1:
            import self_play_TS as self_play
        else:
            pass
        print("\nHere's a list of games to pick:")
        ...
        while True:
            ...
            if choice == 0:
                ...
                Fstd = nevergrad.p.Log(lower=0.1, upper=0.5)
                g_std = nevergrad.p.Log(lower=4.0, upper=8.0)
                lr_init = nevergrad.p.Log(lower=0.0001, upper=0.1)
                discount = nevergrad.p.Log(lower=0.95, upper=0.9999)
                parametrization = nevergrad.p.Dict(lr_init=lr_init, discount=discount, Fstd=Fstd, g_std=g_std)
